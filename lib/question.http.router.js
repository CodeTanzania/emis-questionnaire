 ...
 *
 * @apiDescription A representation of question that is used to assess need,
 ...
 * @apiSuccess {Indicator} indicator Indicator which a question assess.
 ...
 * @apiSuccess {Indicator} data.indicator Indicator which a question assess.
...
router.get(PATH_LIST, function getQuestions(request, response, next) {
  ...
  Question.get(options, function onGetQuestions(error, results) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.post(PATH_LIST, function postQuestion(request, response, next) {
  // obtain request body
  const body = _.merge({}, request.body);

  Question.post(body, function onPostQuestion(error, created) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.get(PATH_SINGLE, function getQuestion(request, response, next) {
  // obtain request options
  const options = _.merge({}, request.mquery);
  ...
  Question.getById(options, function onGetQuestion(error, found) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.patch(PATH_SINGLE, function patchQuestion(request, response, next) {
  // obtain question id
  ...
  Question.patch(id, patches, function onPatchQuestion(error, patched) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.put(PATH_SINGLE, function putQuestion(request, response, next) {
  // obtain question id
  const { id } = request.params;
  ...
  Question.put(id, updates, function onPutQuestion(error, updated) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.delete(PATH_SINGLE, function deleteQuestion(request, response, next) {
  // obtain question id
  const { id } = request.params;

  Question.del(id, function onDeleteQuestion(error, deleted) {
    // forward error
    if (error) {
      next(error);
    }
