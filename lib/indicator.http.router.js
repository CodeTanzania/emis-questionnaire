...
router.get(PATH_LIST, function getIndicators(request, response, next) {
  ...
  Indicator.get(options, function onGetIndicators(error, results) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.post(PATH_LIST, function postIndicator(request, response, next) {
  // obtain request body
  const body = _.merge({}, request.body);

  Indicator.post(body, function onPostIndicator(error, created) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.get(PATH_SINGLE, function getIndicator(request, response, next) {
  // obtain request options
  const options = _.merge({}, request.mquery);
  ...
  Indicator.getById(options, function onGetIndicator(error, found) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.patch(PATH_SINGLE, function patchIndicator(request, response, next) {
  // obtain indicator id
  ...
  Indicator.patch(id, patches, function onPatchIndicator(error, patched) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.put(PATH_SINGLE, function putIndicator(request, response, next) {
  // obtain indicator id
  const { id } = request.params;
  ...
  Indicator.put(id, updates, function onPutIndicator(error, updated) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.delete(PATH_SINGLE, function deleteIndicator(request, response, next) {
  // obtain indicator id
  const { id } = request.params;

  Indicator.del(id, function onDeleteIndicator(error, deleted) {
    // forward error
    if (error) {
      next(error);
    }
